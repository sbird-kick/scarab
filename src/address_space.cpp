...
void decode_all(std::string trace_file, std::string xedPath)
{
    ...
    std::unordered_map<std::string, unsigned long long> instructionPairCounters; // Occurances of each instruction pair combination for: MOV, ALU, PUSH, POP, PREFETCH, LOCK, CALL, NOP.
    std::unordered_map<std::string, unsigned long long> userSpaceCount; // Both instructions are in user space
    std::unordered_map<std::string, unsigned long long> kernelSpaceCount; // Both instructions are in kernel space
    std::unordered_map<std::string, unsigned long long> mixedSpaceCount; // One instruction is in user space and the other is in kernel space
    ...
    while (read_traces(addresses, hexes, trace_file))
    {
        ...
        for(short k = 0; k < addresses.size(); k++)
        {
            ...
            else 
            {
                ...
                // Determine the space of the instructions
