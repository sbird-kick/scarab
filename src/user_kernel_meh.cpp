...
{
    std::cout << "Reading traces from " << traceFileName << std::endl;
...
{
    std::cout << "Decoding traces" << std::endl;
...
{
    std::cout << "Starting full decode process for trace file: " << trace_file << std::endl;

    unsigned long long KERNEL_SPACE_START = 0xffff800000000000ull;
    unsigned long long KERNEL_SPACE_END = 0xffffffffffffffffull;

    bool isFirstInstructionOfPair = true;  
    std::string prevAddr = "";
    std::string currAddr = "";
    std::string prevIclass = "";
    std::string currIclass = "";

    std::unordered_map<std::string, unsigned long long> instructionPairCounters;
    std::unordered_map<std::string, unsigned long long> userSpaceCount;
    std::unordered_map<std::string, unsigned long long> kernelSpaceCount;
    std::unordered_map<std::string, unsigned long long> mixedSpaceCount;
    ...
    while (read_traces(addresses, hexes, trace_file))
    {
        ...
        for (short k = 0; k < addresses.size(); k++)
        {
            ...
            if (isFirstInstructionOfPair)
            {
                
                prevAddr = addresses[k];
                prevIclass = xed_iclass;
                isFirstInstructionOfPair = false;
            }
            else
            {
                currAddr = addresses[k];
                currIclass = xed_iclass;

                if (prevAddr.empty() || currAddr.empty())
                {
                    std::cerr << "Empty address encountered: prevAddr='" << prevAddr << "', currAddr='" << currAddr << "'\n";
                    prevAddr = currAddr;
                    prevIclass = currIclass;
                    continue;
                }

                try
                {
                    unsigned long long prevAddrHex = std::stoull(prevAddr, nullptr, 16);
                    unsigned long long currAddrHex = std::stoull(currAddr, nullptr, 16);

                    std::string prevSpace = (prevAddrHex >= KERNEL_SPACE_START && prevAddrHex <= KERNEL_SPACE_END) ? "K" : "U";
                    std::string currSpace = (currAddrHex >= KERNEL_SPACE_START && currAddrHex <= KERNEL_SPACE_END) ? "K" : "U";

                    std::string iClassPair = prevIclass + "_" + currIclass;

                    if (prevSpace == "U" && currSpace == "U")
                    {
                        userSpaceCount[iClassPair]++;
                    }
                    else if (prevSpace == "K" && currSpace == "K")
                    {
                        kernelSpaceCount[iClassPair]++;
                    }
                    else
                    {
                        mixedSpaceCount[iClassPair]++;
                    }

                    instructionPairCounters[iClassPair]++;
                }
                catch (const std::invalid_argument& e)
                {
                    std::cerr << "Invalid address format for: prevAddr='" << prevAddr << "', currAddr='" << currAddr << "'\n";
                    prevAddr = currAddr;
                    prevIclass = currIclass;
                    continue;
                }
                ...
                {
                    std::cerr << "Address out of range for: prevAddr='" << prevAddr << "', currAddr='" << currAddr << "'\n";
                    prevAddr = currAddr;
                    prevIclass = currIclass;
                    continue;
                }
    ...
    std::cout << "Instruction Pair Counters (Multiply by 2 for total occurrences)" << std::endl;
    for (auto& pair : instructionPairCounters)
    {
        std::cout << pair.first << " : " << pair.second << std::endl;
    }

    std::cout << "User Space Count (Multiply by 2 for total occurrences)" << std::endl;
    for (auto& pair : userSpaceCount)
    {
        std::cout << pair.first << " : " << pair.second << std::endl;
    }

    std::cout << "Kernel Space Count (Multiply by 2 for total occurrences)" << std::endl;
    for (auto& pair : kernelSpaceCount)
    {
        std::cout << pair.first << " : " << pair.second << std::endl;
    }

    std::cout << "Mixed Space Count (Multiply by 2 for total occurrences)" << std::endl;
    for (auto& pair : mixedSpaceCount)
    {
        std::cout << pair.first << " : " << pair.second << std::endl;
    }
